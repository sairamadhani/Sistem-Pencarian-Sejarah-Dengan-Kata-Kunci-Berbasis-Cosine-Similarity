...
data_cleaned.loc[:, "cleaned_title"] = data_cleaned["title"].apply(clean_text)
...
# Menggunakan .loc[] untuk menghindari SettingWithCopyWarning
data_cleaned.loc[:, 'tokenized_title'] = data_cleaned['cleaned_title'].apply(word_tokenize)
data_cleaned.loc[:, 'lemmatized_title'] = data_cleaned['tokenized_title'].apply(lemmatize_tokens)
data_cleaned.loc[:, 'lemmatized_text'] = data_cleaned['lemmatized_title'].apply(lambda x: ' '.join(x))
